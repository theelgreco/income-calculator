...
const { signUp, login, signInWithGoogle, googleRedirect } = useUserStore();
        ...
        <div class="bg-[#F8F8F8] shadow-2xl flex flex-col gap-15 w-full lg:w-1/2 my-30 py-25 px-35 border rounded-3xl">
            ...
            <div class="flex flex-col gap-4 flex-grow p-15 shadow-xl bg-white rounded-xl">
                <div class="relative w-full items-center">
                    <Input v-model="email" type="email" placeholder="Email" class="pl-10 py-5 rounded-xs" />
                    <span class="absolute start-0 inset-y-0 flex items-center justify-center px-3">
                        <Mail class="size-4 text-muted-foreground/50" />
                    </span>
                </div>
                <div class="relative w-full items-center">
                    <Input v-model="password" type="password" placeholder="Password" class="pl-10 py-5 rounded-xs" />
                    <span class="absolute start-0 inset-y-0 flex items-center justify-center px-3">
                        <Lock class="size-4 text-muted-foreground/50" />
                    </span>
                </div>
                <div class="flex flex-col mt-4 gap-3">
                    <Button class="rounded-xs py-5" @click="handleLogin">Login</Button>
                    <Button variant="outline" class="rounded-xs py-5" @click="handleSignUp">Sign up</Button>
                </div>
